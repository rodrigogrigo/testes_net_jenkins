...
namespace RgCidadao.Domain.Infra.Repositories.Cadastro
{
    public class CidadaoRepository : ICidadaoRepository
    {
        ...
        public List<Cidadao> GetAllPagination(string ibge, string filtro, int page, int pagesize, string sql_estrutura)
        {
            try
            {
                ...
                if (string.IsNullOrWhiteSpace(filtro))
                {
                    var sql = _cidadaocommand.GetAllPagination.Replace("@filtro", "").Replace("@sql_estrutura", sql_estrutura);
                    cid = Helpers.HelperConnection.ExecuteCommand(ibge, conn =>
                          conn.Query<Cidadao>(sql, new { @pagesize = pagesize, @page = page }).ToList());
                }
                else
                {
                    var sql = _cidadaocommand.GetAllPagination.Replace("@filtro", $" {filtro} ").Replace("@sql_estrutura", sql_estrutura);
                    cid = Helpers.HelperConnection.ExecuteCommand(ibge, conn =>
                          conn.Query<Cidadao>(sql, new { @pagesize = pagesize, @page = page }).ToList());
                }
                ...
                foreach (var item in cid)
                {
                    ...
                    item.idade = $@"{anos_idade.Item1} Anos {anos_idade.Item2} Meses e {anos_idade.Item3} dias";
                    item.idadeAnos = anos_idade.Item1;
                }
        ...
        public Cidadao GetCidadaoById(string ibge, int id, string sql_estrutura)
        {
            try
            {
                var sql = _cidadaocommand.GetCidadaoById.Replace("@sql_estrutura", sql_estrutura);
                var cidadao = Helpers.HelperConnection.ExecuteCommand(ibge, conn =>
                        conn.QueryFirstOrDefault<Cidadao>(sql, new { @csi_codpac = id }));
        ...
        public List<Cidadao> ValidaExistenciaCidadaoParam(string ibge, string filtro)
        {
            try
            {
                var cidadao = Helpers.HelperConnection.ExecuteCommand(ibge, conn =>
                                         conn.Query<Cidadao>(_cidadaocommand.ValidaExistenciaCidadao.Replace("@filtro", filtro)).ToList());
                return cidadao;
            }
            catch (Exception ex)
            {
        ...
        public List<CidadaoFamiliaViewModel> GetAllPaginationWithFamilia(string ibge, int page, int pagesize, string filtro)
        {
            try
            {
                string sql = string.Empty;
                if (!string.IsNullOrWhiteSpace(filtro))
                    sql = _cidadaocommand.GetAllPaginationWithFamilia.Replace("@filtro", filtro);
                else
                    sql = _cidadaocommand.GetAllPaginationWithFamilia.Replace("@filtro", string.Empty);

                var cidadao = Helpers.HelperConnection.ExecuteCommand(ibge, conn =>
                                        conn.Query<CidadaoFamiliaViewModel>(sql, new
                                        {
                                            @pagesize = pagesize,
                                            @page = page
                                        }).ToList());
        ...
        public int GetCountAllWithFamilia(string ibge, string filtro)
        {
            try
            {
                string sql = string.Empty;
                if (!string.IsNullOrWhiteSpace(filtro))
                    sql = _cidadaocommand.GetCountAllWithFamilia.Replace("@filtro", filtro);
                else
                    sql = _cidadaocommand.GetCountAllWithFamilia.Replace("@filtro", string.Empty);
        ...
        public List<CidadaoFamiliaViewModel> GetIndividuosToFamilia(string ibge, int page, int pagesize, string filtro)
        {
            try
            {
                string sql = string.Empty;
                if (!string.IsNullOrWhiteSpace(filtro))
                    sql = _cidadaocommand.GetIndividuosToFamilia.Replace("@filtro", filtro);
                else
                    sql = _cidadaocommand.GetIndividuosToFamilia.Replace("@filtro", string.Empty);
        ...
        public int GetCountIndividuosToFamilia(string ibge, string filtro)
        {
            try
            {
                string sql = string.Empty;
                if (!string.IsNullOrWhiteSpace(filtro))
                    sql = _cidadaocommand.GetCountIndividuosToFamilia.Replace("@filtro", filtro);
                else
                    sql = _cidadaocommand.GetCountIndividuosToFamilia.Replace("@filtro", string.Empty);
        ...
        public List<CidadaoViewModel> GetCidadaoByIdProntuarioIdAgente(string ibge, int? id_prontuario, int id_agente)
        {
            try
            {
                string sql = _cidadaocommand.GetCidadaoByIdProntuarioIdAgente;
                if (id_prontuario != null)
                {
                    sql += $@" AND F.num_prontuario_familiar = " + id_prontuario;
                }
                var cidadao = Helpers.HelperConnection.ExecuteCommand(ibge, conn =>
                                       conn.Query<CidadaoViewModel>(sql, new
                                       {
                                           @id_profissional = id_agente
                                       }).ToList());
                return cidadao;
            }
            catch (Exception ex)
        ...
        public bool VerificaExisteEsusFamilia(string ibge)
        {
            try
            {
                string sql = _cidadaocommand.VerificaExisteEsusFamilia;
