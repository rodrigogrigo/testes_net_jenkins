    ...
    public class ConsultaCadSus
    {
        public List<PacientesCNSViewModel> ConsultaDadosCidadao(string nome, string cns, string sexo, string datanasc, string nomemae, string LoginCadSus, string SenhaCadSus)
        {
            try
            {
                ...
                string path =Directory.GetCurrentDirectory() + $"\\wwwroot\\Servico\\RGSvrCadSus_.jar";
                var caminho = Directory.GetCurrentDirectory() + $"\\wwwroot\\Servico\\Consultas\\ConsultaCNS{DateTime.Now.ToString("yyyyMMdd HHmmss")}.txt";
